...
const GitHubProjects = ({ limit }) => {
  ...
  const translations =
    language === "fr"
      ? require("../../locales/fr").default
      : require("../../locales/en").default;

  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await fetch(
          "https://api.github.com/users/killianmathias/repos"
        );
        ...
        const sorted = data
          .sort((a, b) => {
            // Trier d'abord par étoiles décroissantes
            if (b.stargazers_count !== a.stargazers_count) {
              return b.stargazers_count - a.stargazers_count;
            }
            // Si égalité sur les étoiles, trier par date de push décroissante
          ...
          sorted.map(async (project) => {
            try {
              ...
              if (!translationResponse.ok) {
                console.warn(
                  `⚠️ Translation file not found for ${project.name} at ${translationUrl}`
                );
              ...
              const translation =
                translationData[language] || translationData["en"];
              return {
                ...project,
              ...
              console.error(
                `❌ Error fetching translation for ${project.name}:`,
                error
              );
              return {
                ...project,
